...
def build_generated(path, build_readme=True, include_readme=False, include_source=True):
    # preparation
    ...
    for yaml_file in collect_filenames('Building', path, INPUT_EXTENSIONS):
        ...
        if build_readme:
            ...
            with open_file_append(path / README_FILENAME) as out:
                if include_readme:
...
def restore_generated(path: Path, branch='', include_readme=True):
    ...
    if include_readme:
        filename_list.append(path / README_FILENAME)
    # restore files
...
def main():
    ...
    if args.action == 'build':
        build_generated(path)
    elif args.action == 'clean':
        clean_generated(path)
    elif args.action == 'compare' or args.action == 'diff':
        compare_generated(path, args.branch, args.compare_graphviz_output)
    elif args.action == 'restore':
        restore_generated(path, args.branch)
